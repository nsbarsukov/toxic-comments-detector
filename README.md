...
```
В данном примере показывается, что модель с 95% вероятностью уверена, что текст `ты чего берега попутал?`,
а уверенность модели, что текст `это правый берег реки, не путай с левым` может быть токсичным составляет лишь 18%.
